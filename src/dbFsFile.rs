...
pub fn insert_fs_file(conn: &Connection, fk_fs_dir: i32, name: String) -> Result<i32, &'static str> {
    let me = FsFile {
        id: 0,
        name: name,
        fk_fs_dir : fk_fs_dir,
    };
    let variable_pair_instance = conn.execute("INSERT INTO FS_FILE (name, fk_fs_dir)
                  VALUES (?1, ?2)",
...
pub fn list_fs_file_type(conn: &Connection, fk_fs_type: &i32) -> Vec<FsFile> {
    let mut stmt = conn.prepare("SELECT FS_FILE.id, FS_FILE.name, FS_FILE.fk_fs_dir FROM FS_FILE
        INNER JOIN FS_DIR
