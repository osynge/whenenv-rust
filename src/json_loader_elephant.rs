...
pub fn json_loader_elephant(conn: &Connection, pk_file: &i32, json :&rustc_serialize::json::Json) {
    ...
    if json.is_object() {
        ...
        for item in job_vaiable_provides {
            ...
            let job_vaiable_provides_pk = elephant::elephant_job_require_variables(conn, &pk_job, &pk_variable_name);
        ...
        for item in job_vaiable_depends {
            ...
            let variable_pair_pk = elephant::elephant_variable_pair_pk(&conn, &pk_variable_name, &item);
        ...
        for item in job_depends {
            ...
            let pk_job_depend = elephant::elephant_job_depend_pk(conn, &pk_job, &item_pk, &order_job_depend);
        ...
        for item in job_requires_vaiable_pair {
            ...
            let pk_variable_pair = elephant::elephant_variable_pair_pk(conn, &pk_variable_name, &item.value);
            let pk_variable_pair_dep = elephant::elephant_job_depend_pair_pk(conn, &pk_job, &pk_variable_pair);
